...
public class LineChartRenderer: LineScatterCandleRadarChartRenderer
{
    ...
    internal func drawCubicFill(context: CGContext?, dataSet: LineChartDataSet, spline: CGMutablePath, matrix: CGAffineTransform, from: Int, to: Int)
    {
        if to - from <= 1
        {
            return
    ...
    private func generateFilledPath(entries: [ChartDataEntry], fillMin: CGFloat, from: Int, to: Int, matrix: CGAffineTransform) -> CGPath
    {
        let phaseX = _animator.phaseX
        let phaseY = _animator.phaseY
        var matrix = matrix
        ...
        if (CGFloat(lineData.yValCount) < CGFloat(delegate!.lineChartRendererMaxVisibleValueCount(self)) * viewPortHandler.scaleX)
        {
            ...
            for i in 0 ..< dataSets.count
            {
                ...
                for j in 0 ..< count
                {
                    ...
                    ChartUtils.drawText(context, text: formatter!.stringFromNumber(val)!, point: CGPoint(x: positions[j].x, y: positions[j].y - CGFloat(valOffset) - valueFont.lineHeight), align: .Center, attributes: [NSFontAttributeName: valueFont, NSForegroundColorAttributeName: valueTextColor])
                }
            }
        }
    }
