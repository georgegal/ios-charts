...
open class ChartYAxisRendererHorizontalBarChart: ChartYAxisRenderer
{
    ...
    open override func renderAxisLabels(_ context: CGContext?)
    {
        ...
        drawYLabels(context, fixedPosition: yPos, positions: positions, offset: _yAxis.yOffset)
    }
    ...
    internal func drawYLabels(_ context: CGContext?, fixedPosition: CGFloat, positions: [CGPoint], offset: CGFloat)
    {
        ...
        for i in 0 ..< _yAxis.entryCount
        {
            ...
            ChartUtils.drawText(context, text: text, point: CGPoint(x: positions[i].x, y: fixedPosition - offset), align: .center, attributes: [convertFromNSAttributedStringKey(NSAttributedString.Key.font): labelFont, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): labelTextColor])
        }
    }
