...
open class ChartLegend: ChartComponentBase
{
    ...
    open func calculateDimensions(_ labelFont: UIFont, viewPortHandler: ChartViewPortHandler)
    {
    ...
    open func setExtra(_ colors: [UIColor?], labels: [String?])
    {
    ...
    open func setCustom(_ colors: [UIColor?], labels: [String?])
    {
        self.labels = labels
        self.colors = colors
    ...
    /// colors and labels that will be appended to the end of the auto calculated colors and labels after calculating the legend.
    /// (if the legend has already been calculated, you will need to call `notifyDataSetChanged()` to let the changes take effect)
    open func setExtra(_ colors: [NSObject], labels: [NSObject])
    {
    ...
    open func setCustom(_ colors: [NSObject], labels: [NSObject])
    {
        if (colors.count != labels.count)
        {
