...
open class BubbleChartRenderer: ChartDataRendererBase
{
    ...
    fileprivate func getShapeSize(_ entrySize: CGFloat, maxSize: CGFloat, reference: CGFloat) -> CGFloat
    {
    ...
    internal func drawDataSet(_ context: CGContext?, dataSet: BubbleChartDataSet)
    {
        ...
        for j in minx ..< maxx
        {
            ...
            let shapeSize = getShapeSize(entry.size, maxSize: dataSet.maxSize, reference: referenceSize)
        ...
        if ((bubbleData?.yValCount)! < Int(ceil(CGFloat(delegate!.bubbleChartRendererMaxVisibleValueCount(self)) * viewPortHandler.scaleX)))
        {
            ...
            for dataSet in dataSets
            {
                ...
                for j in 0 ..< positions.count
                {
                    ...
                    ChartUtils.drawText(context, text: text!, point: CGPoint(x: positions[j].x, y: positions[j].y - ( 0.5 * lineHeight)), align: .center, attributes: [convertFromNSAttributedStringKey(NSAttributedString.Key.font): valueFont, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): valueTextColor])
                }
            }
        }
    }
