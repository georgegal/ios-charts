...
open class ChartData: NSObject
{
    ...
    internal func calcMinMax(_ start: Int, end: Int)
    {
        ...
        else
        {
            ...
            {
                _dataSets[i].calcMinMax(start, end: end)
    ...
    open func removeEntry(_ entry: ChartDataEntry!, dataSetIndex: Int) -> Bool
    {
        ...
        let removed = _dataSets[dataSetIndex].removeEntry(index: entry.xIndex)
    ...
    open func contains(_ entry: ChartDataEntry) -> Bool
    {
        for set in dataSets
        {
